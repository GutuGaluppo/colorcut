...
- The app never talks to Photoroom directly once the proxy exists — it calls only the Worker's URL. This is the first network access this app has ever made, but it needed no new Tauri capability entry: the request happens natively in Rust (`reqwest`, inside `PhotoroomRemovalService`), never through the webview-exposed `http` plugin, and Tauri's capability/ACL system only gates plugin-exposed and JS-invokable surfaces. The least-privilege rule in `AGENTS.md`'s security section is still honored in spirit — this Rust code only ever calls one hardcoded URL, never an arbitrary one.
...
- **Done (2026-09-23):** the Worker (`cloudflare-worker/`) is implemented, deployed, and live at `https://colorcut-photoroom-proxy.colorcutapp.workers.dev` (`PHOTOROOM_PROXY_URL` in `src-tauri/src/lib.rs` points at it), with the Photoroom key provisioned via `wrangler secret` and license codes matched case-insensitively (uppercased before the KV lookup). See `cloudflare-worker/README.md` for the deployment and license-issuance process.
